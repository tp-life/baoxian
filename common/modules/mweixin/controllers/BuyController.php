...
class BuyController extends BaseController
{
	...
	public function actionActive()
	{
		...
		//是否发放给下家
		$hasGrantChildren = CardGrantRelation::find()->where(['card_number' => $status->card_number, 'from_seller_id' => $status->seller_id])->orderBy('id DESC')->one();
		...
		try {
			...
			$order_extend = [
				...
				'buyer' => $post['name'],
				'buyer_phone' => $cache['tel'],
				'imei_code' => $post['imei'],
				'idcrad' => $post['id_card'],
				'brand_id' => $cache['brand'],
				'model_id' => $cache['model']
			];
			if ($hasGrantChildren) {
				$order_extend['seller_id'] = $hasGrantChildren['to_seller_id'];
				$order_extend['seller_name'] = Seller::getSellerInfo($hasGrantChildren['to_seller_id'])->seller_name;
			}
			$model = new OrderExtend();
			$model->setAttributes($order_extend);
			...
			$log = ['hand_type' => CardCouponsLog::__TYPE_ACTIVE,
				'from_seller_id' => $status->seller_id,
				'message' => $post['name'] . '[' . $this->member_id . ']激活卡券(' . $status->coverage_code . ')',
				'to_seller_id' => $status->seller_id
			];
