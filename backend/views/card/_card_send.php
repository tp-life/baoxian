		...
		<form action="#" id="service_form" method="POST" class="form-horizontal">
			<div class="form-body">
				...
				<div class="form-group">
					...
					<div class="col-md-6">
						<input class="form-control" onkeyup="value=value.replace(/[^\d]/g,'') " onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d]/g,''))" type="text" name="deadline" id="deadline" value="" placeholder="请设置最迟回款天数">
						<span class="help-block font-red-pink">申领默认没有设置回款限制</span>
					</div>
				</div>
				<div class="form-group">
			...
			<div class="form-actions">
				<div class="row">
					<div class="col-md-offset-3 col-md-9">
						<button type="button" class="btn green" id="submit_lipei_liucheng" >确认</button>
					</div>
				</div>
			</div>
		</form>
				...
				$(kay_value_set).each(function(key,item){
					...
					if(item.name=='deadline'){
						...
						if(item.value!='' && !re.test(item.value)){
							showToastr('error', '请填写回款天数或者设置零');
							ok = false;
						}
					}
					if(item.name=='card_number_str' && item.value==''){
						showToastr('error', '请填写发放卡券序列号');
						ok = false;
					}
