...
<div class="row">
    <div class="col-md-12">
        <div class="note note-danger">
            <p> 卡券将直接发放至商家.</p>
			<p style="color:orangered"> 商家过滤条件将筛选出的是签约商户 即一级商家.</p>
        </div>
        ...
        <div class="portlet light portlet-fit portlet-datatable bordered">
            ...
            <form id="merge_form" class="form-horizontal" method="post" enctype="multipart/form-data" method="POST">
                ...
                <div class="form-body">
					...
					<div class="form-group  margin-top-20">
						<label class="control-label col-md-3">付款方式
							<span class="required"> * </span>
						</label>
						<div class="col-md-4">
							<div class="input-icon right">
								<i class="fa"></i>
								<?= \yii\helpers\Html::dropDownList('apply_type','3',\common\models\CardOrderPayback::typeData(),['class'=>'form-control form-filter input-sm','id'=>'apply_type']) ?>
							</div>
						</div>
                    ...
                    <div class="form-group  margin-top-20">
                        <label class="control-label col-md-3">回款天数</label>
                        <div class="col-md-4">
                            <div class="input-icon right">
                                <div class="input-group"><input  class="form-control " name="card_deadline"><span class="input-group-addon">天</span></div></div>
                            </div>
                        </div>
                    </div>
                    <div class="form-group  margin-top-20">
...
<script>
    $(function () {
        ...
        $('#to_check').on('click', function () {
			...
			if(seller_name.length<=1){
				showToastr('error','请输入不少于2个字符以便搜索');return false;
			}
				App.startPageLoading();
            var str = '<option value="">选择保险商家</option>';
            $.post('<?= \yii\helpers\Url::to(['card/getseller']) ?>', {
                ...
                function (data) {
                    ...
                    if (data.code == 'yes') {
                        ...
                        $.each(data.data, function (index, val) {
							if(index==0){
								str += '<option selected value=' + val.seller_id + '>' + val.seller_name + '</option>';
							}else{
    ...
    $("#merge_form").validate({
        ...
        messages: {
			...
			card_num:{
				required: '<b style="color: red">请填写发放数量<b/>'
			}
        }
    });
