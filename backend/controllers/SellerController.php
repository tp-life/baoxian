...
class SellerController extends LoginedController
{
    ...
    public function actionGetdata()
    {
        ...
        $s = Seller::tableName();
        $b = Bank::tableName();
        $user = Seller::find()->leftJoin($b . ' as bank', 'bank.member_id = ' . $s . '.member_id')->select($s . '.*,bank.brank_name,bank.account_holder,bank.brank_account');
        ...
        if ($seller_type_flag = Yii::$app->request->post('seller_type', 0)) {
            if ($seller_type_flag == '1') {
                $user->andWhere([$s . '.is_insurance' => 1]);
            } elseif ($seller_type_flag == '2') {
                $user->andWhere([$s . '.is_repair' => 1]);
            }
        }
        $count = $user->count('*');
...
//            in_array('2',$post['is_type']) && $model->is_repair=1;
	...
	public function actionExport()
	{
		...
		$query = Seller::find()->from(['a'=>Seller::tableName()]);
		$query->leftJoin(['b'=>Member::tableName()],'a.member_id=b.member_id');


		if ($filed) {
			$query->andWhere(['or', 'a.seller_name like \'%' . $filed . '%\'',  'a.seller_id = ' . (int)$filed]);
		}
		if ($val) {
			$query->andWhere(['a.status' => $val - 1]);
		}
		if ($p_p == '2') {
			//一级商家
			$query->andWhere(['a.pid' => 0]);
		} elseif ($p_p == '1') {
			//子商家
			$query->andWhere(['<>', 'a.pid', 0]);
		}
		...
		if ($seller_type_flag = Yii::$app->request->get('seller_type', 0)) {
			if ($seller_type_flag == '1') {
				$query->andWhere(['a.is_insurance' => 1]);
			} elseif ($seller_type_flag == '2') {
				$query->andWhere(['a.is_repair' => 1]);
			}
		}
		$count = $query->count('*');
		if($count>5000){
		...
		$data = $query->select('a.*,b.phone as m_phone')->orderBy('a.seller_id DESC')->limit(5000)->asArray()->all();
		...
		theCsv::export([
			...
			'header' => ['商家名称','登录账号','所在区域','联系人','联系电话','合作状态','商家类型','商家等级']
		]);
	}
